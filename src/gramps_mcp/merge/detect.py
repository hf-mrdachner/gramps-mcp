...
def get_birth_year(person_json: dict, conn: sqlite3.Connection) -> Optional[int]:
    """
    Extract birth year from a person's event_ref_list.

    Args:
        person_json: Gramps person JSON dict.
        conn: SQLite connection.

    Returns:
        Birth year as int, or None if not found.
    """
    for eref in person_json.get("event_ref_list", []):
        ...
        if ev.get("type", {}).get("value") == 12:  # Birth
...
def similarity_score(
    ...
    if birth_year_1 and birth_year_2:
    ...
    elif birth_year_1 or birth_year_2:
        score -= 1
...
def find_duplicate_persons(
    ...
    birth_years = {h: get_birth_year(p, conn) for h, p in persons.items()}
    ...
    for handles in groups.values():
        ...
        for i in range(len(handles)):
            for j in range(i + 1, len(handles)):
                ...
                # Assign winner as the richer record
