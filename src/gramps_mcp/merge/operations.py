...
def merge_persons(
    ...
    def _merge_list(field: str) -> None:
        w_refs = {x["ref"] if isinstance(x, dict) else x for x in wp.get(field, [])}
        added = []
        for item in lp.get(field, []):
            ref = item["ref"] if isinstance(item, dict) else item
            if ref not in w_refs:
                if field == "event_ref_list" and ref in events_to_skip:
                    continue
                wp.setdefault(field, []).append(item)
                w_refs.add(ref)
                added.append(ref)
        if added:
            changes.append(f"+{len(added)} {field}")

    def _merge_handle_list(field: str) -> None:
        w_set = set(wp.get(field, []))
        added = [h for h in lp.get(field, []) if h not in w_set]
        for h in added:
            wp.setdefault(field, []).append(h)
            w_set.add(h)
        if added:
            changes.append(f"+{len(added)} {field}")
    ...
    for f in ("event_ref_list", "citation_list", "note_list", "media_list",
              ...
              "person_ref_list", "tag_list"):
        _merge_list(f)
    _merge_handle_list("family_list")
    _merge_handle_list("parent_family_list")
    ...
    if wp["birth_ref_index"] == -1 and lp["birth_ref_index"] != -1:
        erefs = lp.get("event_ref_list", [])
        idx = lp["birth_ref_index"]
        loser_birth_h = erefs[idx]["ref"] if 0 <= idx < len(erefs) else None
        if loser_birth_h not in events_to_delete:
            wp["birth_ref_index"] = idx
            changes.append("adopted birth_ref_index")

    if wp["death_ref_index"] == -1 and lp["death_ref_index"] != -1:
        wp["death_ref_index"] = lp["death_ref_index"]
        changes.append("adopted death_ref_index")
    ...
    conn.execute("BEGIN")
    try:
        _commit_merge(conn, wp, winner_handle, loser_handle, event_updates, events_to_delete)
        conn.execute("COMMIT")
    except Exception:
...
def _commit_merge(
    ...
    Args:
        conn: SQLite connection (BEGIN must be active).
    ...
    """
    surname = ""
    if wp["primary_name"].get("surname_list"):
        surname = wp["primary_name"]["surname_list"][0].get("surname", "")

    conn.execute(
        "UPDATE person SET json_data=?, given_name=?, surname=? WHERE handle=?",
        (json.dumps(wp, ensure_ascii=False),
         wp["primary_name"]["first_name"], surname, winner_handle),
    )

    for ev_handle, ev_data in (event_updates or {}).items():
        conn.execute(
            "UPDATE event SET json_data=? WHERE handle=?",
            (json.dumps(ev_data, ensure_ascii=False), ev_handle),
        )

    for row in conn.execute("SELECT handle, json_data FROM family").fetchall():
    ...
    for ev_handle in (events_to_delete or set()):
...
def _restore_person(conn: sqlite3.Connection, backup: dict) -> bool:
    """
    ...
    Returns:
        True if inserted, False if the handle was already taken.
    """
    ...
    if conn.execute(
        ...
        print(f"  Handle {loser_handle!r} already taken — {loser_id} skipped.")
        return False

    surname = ""
    if lp.get("primary_name", {}).get("surname_list"):
        surname = lp["primary_name"]["surname_list"][0].get("surname", "")
    ...
    print(f"  Restored: {loser_id} ({given} {surname})")
    return True
...
def split_person(
    ...
    if not matching:
        print(f"No merge backup found for {gramps_id}.")
        row = conn.execute(
            "SELECT json_data FROM person WHERE gramps_id=?", (gramps_id,)
        ).fetchone()
        if row is None:
            print(f"Person {gramps_id} not found in database.")
            return
        ...
        if alt_names:
            print(f"Found {len(alt_names)} alternate name(s) — no full restore possible.")
            print("Please review and split manually in Gramps.")
        else:
            print("No alternate names found — nothing to split.")
        return

    print(f"Splitting {len(matching)} person(s) from {gramps_id}...")
    restored = 0
    conn.execute("BEGIN")
    try:
        for rec in matching:
            if _restore_person(conn, rec):
                restored += 1
        conn.execute("COMMIT")
    except Exception:
        conn.execute("ROLLBACK")
    ...
    print(f"Restored: {restored}/{len(matching)} person(s)")
    print("NOTE: Family links and event assignments need manual review in Gramps.")
    print("The winner record still contains all merged data — remove duplicates there.")
