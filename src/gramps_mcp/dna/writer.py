...
def _build_note_text(match: DnaMatch) -> str:
    ...
    parts = []
    if match.source:
        parts.append(f"Src: {match.source}")
    ...
    if match.relationship:
        parts.append(f"Rel: {match.relationship}")
    if match.side:
...
def write_dna_match(
    ...
    with conn:
        conn.execute(
            "INSERT INTO note (handle, gramps_id, json_data, change) VALUES (?,?,?,?)",
            (note_handle, gramps_id, json.dumps(note_json), int(time.time())),
        )
        conn.execute(
