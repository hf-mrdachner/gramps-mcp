...
def _backup_file(client) -> str:
    ...
    return db_path.replace("sqlite.db", "merge_backups.json")
...
async def split_person_tool(arguments: Dict) -> List[TextContent]:
    ...
    try:
        ...
        sys.stdout = buf
        try:
            _split_person(conn, params.gramps_id, backup)
        finally:
            sys.stdout = old_stdout

        output = buf.getvalue().strip()
        return [TextContent(type="text", text=output or f"Split completed for {params.gramps_id}.")]
